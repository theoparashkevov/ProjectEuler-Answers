...
# Check right-down-diagonal: row: 1 thru 16 (index 0:15); columns: 1 thru 16 (index 0:15). row,col increments by 1 to
# go right-down-diagonal
mx = max(np.prod([ss[i+k,j+k] for k in range(4)]) for i in range(16) for j in range(16))

# Check left-down-diagonal: row: 1 thru 20 (index 0:19); columns: 4 thru 20 (index 3:19). row increments by 1 and col
# decrements by 1 to go right-down-diagonal
